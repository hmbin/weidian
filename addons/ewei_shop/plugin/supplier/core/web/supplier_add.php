<?php
global $_W, $_GPC;
$perms = pdo_fetch('select * from ' . tablename('ewei_shop_perm_role') . ' where status1 = 1');
if(empty($perms)){
    $data = array(
        'rolename' => '供应商',
        'status' => 1,
...
if($operation == 'post'){
    ...
    $su_info = pdo_fetch('select * from ' . tablename('ewei_shop_perm_user') . ' where uniacid=:uniacid and id=:id' , array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
        ));
    if (!empty($su_info)) {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        if (!empty($su_info['id'])) {
            ...
            pdo_update('ewei_shop_perm_user', $pwd, array('uid' => $su_info['uid'], 'uniacid' => $_W['uniacid']));
