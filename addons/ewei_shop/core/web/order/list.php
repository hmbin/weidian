		...
		if($perm_role == 1){
            ...
            if(!empty($applytype)){
                ...
                foreach ($mygoodsid as $ids) {
                    ...
                    pdo_update('ewei_shop_order_goods', $arr, array('id' => $ids['id']));
                }
...
function changeWechatSend($val1224, $val1225, $val1226 = '')
{
    ...
    if (!empty($val1228["openid"])) {
        ...
        $val1240 = array("appid" => $val1235["key"], "openid" => $val1228["openid"], "transid" => $val1228["tag"]["transaction_id"], "out_trade_no" => $val1228["plid"], "deliver_timestamp" => TIMESTAMP, "deliver_status" => $val1225, "deliver_msg" => $val1226);
        $val1247 = $val1240;
...
function order_list_confirmsend($val1273)
{
    ...
    pdo_update("ewei_shop_order", array("status" => 2, "express" => trim($_GPC["express"]), "expresscom" => trim($_GPC["expresscom"]), "expresssn" => trim($_GPC["expresssn"]), "sendtime" => time()), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
    if (!empty($val1273["refundid"])) {
        $val1289 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id limit 1", array(":id" => $val1273["refundid"]));
        if (!empty($val1289)) {
            pdo_update("ewei_shop_order_refund", array("status" => -1), array("id" => $val1273["refundid"]));
            pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $val1273["id"]));
        }
    }
...
function order_list_confirmsend1($val1273)
{
    ...
    $val1303 = time();
    $val1304 = array("status" => 3, "sendtime" => $val1303, "finishtime" => $val1303);
    if ($val1273["isverify"] == 1) {
        $val1304["verified"] = 1;
        $val1304["verifytime"] = $val1303;
        $val1304["verifyopenid"] = "";
    }
    pdo_update("ewei_shop_order", $val1304, array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
    if (!empty($val1273["refundid"])) {
        $val1289 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id limit 1", array(":id" => $val1273["refundid"]));
        if (!empty($val1289)) {
            pdo_update("ewei_shop_order_refund", array("status" => -1), array("id" => $val1273["refundid"]));
...
function order_list_cancelsend($val1273)
{
    ...
    pdo_update("ewei_shop_order", array("status" => 1, "sendtime" => 0), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
...
function order_list_cancelsend1($val1273)
{
    ...
    pdo_update("ewei_shop_order", array("status" => 1, "finishtime" => 0), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
...
function order_list_finish($val1273)
{
    global $_W, $_GPC;
    ca("order.op.finish");
    pdo_update("ewei_shop_order", array("status" => 3, "finishtime" => time()), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
    m("member")->upgradeLevel($val1273["openid"]);
    m("notice")->sendOrderMessage($val1273["id"]);
...
function order_list_cancelpay($val1273)
{
    ...
    pdo_update("ewei_shop_order", array("status" => 0, "cancelpaytime" => time()), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
...
function order_list_confirmpay($val1273)
{
    ...
    if (!empty($val1273["virtual"]) && $val1370) {
        ...
        pdo_update("ewei_shop_order", array("status" => 1, "paytype" => 11, "paytime" => time()), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
...
function order_list_close($val1273)
{
    ...
    if ($val1273["refundstate"] > 0 && !empty($val1273["refundid"])) {
        ...
        pdo_update("ewei_shop_order_refund", $val1394, array("id" => $val1273["refundid"], "uniacid" => $_W["uniacid"]));
    }
    if ($val1273["deductcredit"] > 0) {
        $val1402 = m("common")->getSysset("shop");
        m("member")->setCredit($val1273["openid"], "credit1", $val1273["deductcredit"], array('0', $val1402["name"] . "购物返还抵扣积分 积分: {$val1273["deductcredit"]} 抵扣金额: {$val1273["deductprice"]} 订单号: {$val1273["ordersn"]}"));
    }
    if ($val1273["deductcredit2"] > 0) {
        $val1402 = m("common")->getSysset("shop");
        m("member")->setCredit($val1273["openid"], "credit2", $val1273["deductcredit2"], array('0', $val1402["name"] . "购物返还抵扣余额 余额: {$val1273["deductcredit2"]} 订单号: {$val1273["ordersn"]}"));
    }
    if (p("coupon") && !empty($val1273["couponid"])) {
        p("coupon")->returnConsumeCoupon($val1273["id"]);
    }
    m("order")->setStocksAndCredits($val1273["id"], 2);
    pdo_update("ewei_shop_order", array("status" => -1, "refundstate" => 0, "canceltime" => $val1303, "remark" => $val1273["remark"] . "\r\n" . $_GPC["reson"]), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
...
function order_list_refund($val1273)
{
    ...
    $val1289 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id and (status=0 or status>1) order by id desc limit 1", array(":id" => $val1273["refundid"]));
    if (empty($val1289)) {
        pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $val1273["id"], "uniacid" => $_W["uniacid"]));
        message("未找到退款申请，不需处理！");
    }
    if (empty($val1289["refundno"])) {
        $val1289["refundno"] = m("common")->createNO("order_refund", "refundno", "SR");
        pdo_update("ewei_shop_order_refund", array("refundno" => $val1289["refundno"]), array("id" => $val1289["id"]));
    }
    ...
    if ($val1440 == 0) {
        ...
        if ($val1440 == 3) {
            ...
            if ($val1450 == 0) {
                $val1455 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where isdefault=1 and uniacid=:uniacid limit 1", array(":uniacid" => $val1446));
            } else {
                $val1455 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $val1450, ":uniacid" => $val1446));
            }
            if (empty($val1455)) {
                $val1455 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where uniacid=:uniacid order by id desc limit 1", array(":uniacid" => $val1446));
            }
            ...
            pdo_update("ewei_shop_order_refund", $val1394, array("id" => $val1273["refundid"]));
            ...
            if ($val1440 == 5) {
                ...
                if ($val1440 == 10) {
                    ...
                    pdo_update("ewei_shop_order_refund", $val1500, array("id" => $val1273["refundid"], "uniacid" => $val1446));
                    $val1506 = array();
                    $val1506["refundstate"] = 0;
                    $val1506["status"] = 3;
                    $val1506["refundtime"] = $val1303;
                    pdo_update("ewei_shop_order", $val1506, array("id" => $val1273["id"], "uniacid" => $val1446));
                    m("notice")->sendOrderMessage($val1273["id"], true);
                } else {
                    if ($val1440 == 1) {
                        ...
                        $val1525 = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename("ewei_shop_order_goods") . " o left join " . tablename("ewei_shop_goods") . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(":orderid" => $val1273["id"], ":uniacid" => $val1446));
                        $val1528 = 0;
                        ...
                        if ($val1273["paytype"] == 1) {
                            m("member")->setCredit($val1273["openid"], "credit2", $val1523, array(0, $val1402["name"] . "退款: {$val1523}元 订单号: " . $val1273["ordersn"]));
                            $val1548 = true;
                        } else {
                            if ($val1273["paytype"] == 21) {
                        ...
                        if ($val1528 > 0) {
                            m("member")->setCredit($val1273["openid"], "credit1", -$val1528, array(0, $val1402["name"] . "退款扣除购物赠送积分: {$val1528} 订单号: " . $val1273["ordersn"]));
                        }
                        if ($val1273["deductcredit"] > 0) {
                            m("member")->setCredit($val1273["openid"], "credit1", $val1273["deductcredit"], array('0', $val1402["name"] . "购物返还抵扣积分 积分: {$val1273["deductcredit"]} 抵扣金额: {$val1273["deductprice"]} 订单号: {$val1273["ordersn"]}"));
                        }
                        if (!empty($val1541)) {
                            if ($val1273["deductcredit2"] > 0) {
                                m("member")->setCredit($val1273["openid"], "credit2", $val1273["deductcredit2"], array('0', $val1402["name"] . "购物返还抵扣余额 积分: {$val1273["deductcredit2"]} 订单号: {$val1273["ordersn"]}"));
                            }
                        }
                        $val1394["reply"] = '';
                        $val1394["status"] = 1;
                        ...
                        pdo_update("ewei_shop_order_refund", $val1394, array("id" => $val1273["refundid"]));
                        m("notice")->sendOrderMessage($val1273["id"], true);
                        pdo_update("ewei_shop_order", array("refundstate" => 0, "status" => -1, "refundtime" => $val1303), array("id" => $val1273["id"], "uniacid" => $val1446));
                        foreach ($val1525 as $val1530) {
                            $val1613 = pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid " . " where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1", array(":goodsid" => $val1530["id"], ":uniacid" => $val1446));
                            pdo_update("ewei_shop_goods", array("salesreal" => $val1613), array("id" => $val1530["id"]));
                        }
                        plog("order.op.refund", "订单退款 ID: {$val1273["id"]} 订单号: {$val1273["ordersn"]}");
                    } else {
                        if ($val1440 == -1) {
                            pdo_update("ewei_shop_order_refund", array("reply" => $val1442, "status" => -1, "endtime" => $val1303), array("id" => $val1273["refundid"]));
                            m("notice")->sendOrderMessage($val1273["id"], true);
                            plog("order.op.refund", "订单退款拒绝 ID: {$val1273["id"]} 订单号: {$val1273["ordersn"]} 原因: {$val1442}");
                            pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $val1273["id"], "uniacid" => $val1446));
                        } else {
                            if ($val1440 == 2) {
                                $val1541 = 2;
                                $val1394["reply"] = '';
