...
if ($operation == "change") {
	...
	/* 供应商插件 */
    $result = pdo_fetchall("SELECT uid,realname,username FROM " . tablename('ewei_shop_perm_user') . ' where uniacid =' . $_W['uniacid']);
        ...
        if (!empty($id)) {
            ...
            if (count($options) > 0) {
                ...
                $html = '';
                $html .= '<table class="tabletable-bordered table-condensed">';
                $html .= "<thead>";
		...
		if(p('supplier')){
			...
			$suproleid = pdo_fetchcolumn('select id from' . tablename('ewei_shop_perm_role') . ' where status1 = 1');
			$userroleid = pdo_fetchcolumn('select roleid from ' . tablename('ewei_shop_perm_user') . ' where uid=:uid and uniacid=:uniacid',array(':uid' => $_W['uid'],':uniacid' => $_W['uniacid']));
			
			if((!empty($userroleid)) && ($userroleid == $suproleid)){
				$sql = 'SELECT * FROM ' . tablename('ewei_shop_goods') . $condition . ' and supplier_uid='.$_W['uid'].' ORDER BY `status` DESC, `displayorder` DESC,
						`id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$sqls = 'SELECT COUNT(*) FROM ' . tablename('ewei_shop_goods') . $condition . ' and supplier_uid='.$_W['uid'];
				...
				$sql = 'SELECT * FROM ' . tablename('ewei_shop_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
							`id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$sqls   = 'SELECT COUNT(*) FROM ' . tablename('ewei_shop_goods') . $condition;
				$total = pdo_fetchcolumn($sqls, $params);
			}
        ...
        if (!empty($total)) {
			...
			if((!empty($userroleid)) && ($userroleid == $suproleid)){
				$sql = "SELECT * FROM " . tablename("ewei_shop_goods") . $condition . " and supplier_uid=".$_W['uid']." ORDER BY `status` DESC, `displayorder` DESC, `id` DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize;
			}else{
