    ...
    if ($operation == "display") {
        ...
        foreach ($list as &$r) {
            if (!empty($r["optionid"])) {
                $optionid = $r["optionid"];
                $goodsid = $r["goodsid"];
                $option = pdo_fetch("select thumb,specs from " . tablename("ewei_shop_goods_option") . " " . " where id=:id and uniacid=:uniacid and goodsid=:goodsid limit 1 ", array(":id" => $optionid, ":uniacid" => $uniacid, ":goodsid" => $goodsid));
                if (!empty($option)) {
